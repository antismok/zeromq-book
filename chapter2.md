...
## Однонаправленные транспорты (Unicast Transports)
ZeroMQ предоставляет набор unicast(однонаправленные) транспортов (inproc, ipc, and tcp) и набор multicast(широковещательных) транспортов (epgm, pgm). Multicast это более продвинутая техника которую мы рассмотрим позже. Даже не вздумайте использовать его пока вам хватает 1-к-N unicast.

Для большинства известных задач используйте tcp, с разорванным(disconnected) tcp транспортом. Это гибко, компактно и быстро для большинства случаев. Мы называем его разорваным (disconnected), так как ZeroMQ tcp транспорт не требует существование конечной точки, пока вы не подключитесь к нему. Клиент и сервер могут подключаться и начинать прослушивать в любое удобное время, могут уходить и приходить когда им вздумается и все это будет происзодить прозрачно для приложения.

Inter-process (межпроцесный) ipc протокол разорванный как и tcp. Он имеет всего одно ограничение - пока не работает под windows. По соглашению мы будем использовать расширение ".ipc" в именовании конечных точек, чтобы избежать потенциальных конфликтов с другими файлами. В юникс системах, если вы используете конечные точки ipc, вам необходимо создавать их с конкретными расширениями иначе они могут быть не доступны приложениям запущеным от разных пользователей. Так же вам нужно убедиться, что все процессы имеют доступ к файлам, если они например работают из одного каталога.

Inter-thread(межпотоковый) транспорт, inproc, является зависимым от соединения. Он гораздо быстрее чем tcp и ipc. Он имеет специфичные ограничений по сравнению с tcp и ipc: Сервер должен начать прослушивать сокет до того как клиент попытается подключиться. Это ограничение могут убрать в будующих версия, но на данный момент это ограничение стоит иметь ввиду. Мы создаем сокет и начинаем его прослушку и только после этого запускаем дочерние потоки которые создают и подключают другие сокеты.