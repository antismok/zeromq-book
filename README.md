...
Если вы форкаете свои процессы, то каждый из процессов потомков должен иметь свой отдельный контекст. Если вы вызывает zmq_ctx_new() до форка, то можно не беспокоиться, каждый из потомков получит свой собственный контекст. В основном все интересные процессы протекают в процессах потомках, а родительский процесс просто ими управляет.
...
Вуаля! Это бесчеловечно, заставлять каждого программиста изобретать свой велосипед по завершению программ, но пользуясь языками с автоочисткой, такие танцы с бубном становятся не нужны.
